include_controls 'canonical-ubuntu-16.04-lts-stig-baseline' do
  ...
  control 'V-75443' do
    ...
    sessions to one for all accounts and/or account types."
    desc "check", "Verify that the Ubuntu operating system limits the number of
    concurrent sessions to \"1\" for all accounts and/or account types by running
    ...
    * hard maxlogins 1
    ...
    If the \"maxlogins\" item is missing or the value is not set to \"1\",
  ...
  control 'V-75449' do
    ...
    requiring that at least three upper-case characters be used."
    desc "check", "Verify the Ubuntu operating system enforces password complexity
    by requiring that at least three upper-case characters be used.
    ...
    If the \"ucredit\" parameter is not equal to \"-3\", or is commented out, this
    is a finding."
    desc "fix", "Configure the Ubuntu operating system to enforce password
    complexity by requiring that at least three upper-case characters be used.
  ...
  control 'V-75451' do
    title "The Ubuntu operating system must enforce password complexity by
    requiring that at least three lower-case characters be used."
    desc "check", "Verify the Ubuntu operating system enforces password complexity
    by requiring that at least three lower-case characters be used.
    ...
    If the \"lcredit\" parameter is not equal to \"-3\", or is commented out, this
    is a finding."
    desc "fix", "Configure the Ubuntu operating system to enforce password
    complexity by requiring that at least three lower-case characters be used.
  ...
  control 'V-75453' do
    title "The Ubuntu operating system must enforce password complexity by
    requiring that at least three numeric characters be used."
    desc "check", "Verify the Ubuntu operating system enforces password complexity
    by requiring that at least three numeric characters be used.
    ...
    If the \"dcredit\" parameter is not equal to \"-3\", or is commented out, this
    is a finding."
    desc "fix", "Configure the Ubuntu operating system to enforce password
    complexity by requiring that at least three numeric characters be used.
  ...
  control 'V-75455' do
    title "All passwords must contain at least three special characters."
    desc "check", "Verify the Ubuntu operating system enforces password complexity
    by requiring that at least three special characters be used.
    ...
    is a finding."
    desc "fix", "Configure the Ubuntu operating system to enforce password
    complexity by requiring that at least three special characters be used.
  ...
  control 'V-75457' do
    title "The Ubuntu operating system must require the change of at least 12
    ...
    require the change of at least 12 characters."
    desc "check", "Verify the Ubuntu operating system requires the change of at
    least \"12\" characters when passwords are changed.
    ...
    If the \"difok\" parameter is less than \"12\", or is commented out, this is a
    finding."
    desc "fix", "Configure the Ubuntu operating system to require the change of at
    least \"12\" characters when passwords are changed.
    
    Add or update the following line in the \"/etc/security/pwquality.conf\" file
    to include the \"difok=12\" parameter:
  ...
  control 'V-75475' do
    ...
    being reused for a minimum of twelve generations by running the following command:
    
    # grep -i remember /etc/pam.d/common-password
    
    password [success=1 default=ignore]      pam_unix.so obscure sha512 remember=12
    rounds=5000
    
    If the \"remember\" parameter value is not greater than or equal to \"12\", is
    ...
    being reused for a minimum of twelve generations.
  ...
  control 'V-75485' do
    ...
    application identifiers after 30 days of inactivity."
    desc "check", "Verify the account identifiers (individuals, groups, roles, and
    devices) are disabled after \"30\" days of inactivity with the following
    command:
    ...
    If \"INACTIVE\" is not set to a value \"0<[VALUE]<=30\", or is commented out,
    this is a finding."
    desc "fix", "Configure the Ubuntu operating system to disable account
    identifiers after 30 days of inactivity after the password expiration.
  ...
  control 'V-75491' do
    ...
    CMS-defined time period of 30 days.
    ...
    Verify each of these accounts has an expiration date set within 30 days.
    ...
    terminate the account after a 30 day time period with the following command to
    set an expiration date on it. Substitute \"system_account_name\" with the
    account to be created.
    
    # sudo chage -E `date -d \"+30 days\" +%Y-%m-%d` system_account_name"
  end
  control 'V-75505' do
  ...
  control 'V-75623' do
    ...
    allocated audit record storage volume reaches 80% of the repository maximum
    audit record storage capacity."
    desc "If security personnel are not notified immediately when storage volume
    reaches 80% utilization, they are unable to plan for audit record storage
    capacity expansion."
    ...
    minimum) via email when allocated audit record storage volume reaches 80% of
    ...
    via email when allocated audit record storage volume reaches 80% of the
    ...
    reaches 80% of the repository maximum audit record storage capacity.
  ...
  control 'V-75813' do
    ...
    time servers, or a time server designated for the appropriate CMS network."
    ...
    Verify that the \"ntp.conf\" file is configured to an authoritative CMS time
    source by running the following command:
    
    # grep -i server /etc/ntp.conf
    server 0.us.pool.ntp.org iburst
    
    If the parameter \"server\" is not set, is not set to an authoritative CMS time
    ...
    with an authoritative CMS time source.
  ...
  control 'V-80961' do
    ...
    audit record storage volume reaches 80% of the repository maximum audit record
    storage capacity."
    desc "If security personnel are not notified immediately when storage volume
    reaches 80% utilization, they are unable to plan for audit record storage
    capacity expansion."
    desc "check", "Verify the Ubuntu operating system notifies the System
    Administrator (SA) and Information System Security Officer (ISSO) (at a
    minimum) when allocated audit record storage volume reaches 80% of the
    ...
    Determine what the threshold is for the system to take action when 80% of the
    repository maximum audit record storage capacity is reached:
    
    # grep -i space_left /etc/audit/auditd.conf
    space_left = 250
    
    If the value of the \"space_left\" keyword is not set to 20% of the total
    partition size, this is a finding."
    desc "fix", "Configure the operating system to immediately notify the SA and
    ISSO (at a minimum) when allocated audit record storage volume reaches 80% of
    the repository maximum audit record storage capacity.
